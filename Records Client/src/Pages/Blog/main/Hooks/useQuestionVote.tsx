...
const useQuestionVote = (Poll:IQuestion) => {
    ...
    let QState=useAppSelector(state=>state.Blog.data?.Question)
        let {mutate,isLoading}=useMutation({mutationKey:"PollingVote",mutationFn:(title:string)=>VoteQ(Poll,title) ,onError(err:any) {
            toast.error(err?.response?.data?.msg)
          }, onSuccess(data) {
            if (QState?.total==0) {
              let UpdatedPoll={...QState,Polled:true,
                PolledTotal:QState?.total||0+1 ,voted:data.title, options:QState?.options.map(elm=>{
  ...
  return elm
              ...
              dispatch(BlogInsert({Polled:true,QuestionVoted:data.title,PolledTotal:QState?.total||0+1}))
            }
          },})
